...
interface Props {
  ...
  iconInvert?: boolean
  /**
   * Warnung: roter Rand statt des ueblichen - bei Kartenpinnadeln, auf deren Karte Gegner
   * stehen. Der Kopf behaelt seine Farbe, damit die Nadel als Karte erkennbar bleibt.
   */
  alert?: boolean
  /**
...
export function MapPin({
  ...
  return (
    <div
      className={`map-pin${selected ? ' is-selected' : ''}${draggable ? ' is-draggable' : ''}${isMapLink ? ' is-map-link' : ''}${iconInvert ? ' is-icon-invert' : ''}${alert ? ' is-alert' : ''}${image ? ' has-image' : ''}${ghost ? ' is-ghost' : ''}${emphasized ? ' is-emphasized' : ''}`}
